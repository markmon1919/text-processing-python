...
class SupplyTool():
	...
	def __init__(self):
		...
		self.fileLs = os.listdir(self.path)
		self.xlsLs = []
		self.csvLs = []
		self.supplyCSV = None
		self.hcCSV = None
	...
	def convert(self):
		#Renaming file extension case and Converting to CSV UTF-8
		for f in self.fileLs:
			if f.endswith('.XLS') or f.endswith('.XLSX'):
				os.rename(f, f.replace(f[f.index('.'):], f[f.index('.'):].lower()))
		...
		for sheet in self.xlsLs:
			if 'Deleted' in sheet:
				...
				supplyF.to_csv(sheet.replace(sheet[sheet.index('.'):], '') + '.csv', sep=',', encoding='UTF-8')
			elif 'HC' in sheet:
				print('  [*]', sheet)
				hcF = pd.read_excel(sheet, 'Sheet1', index_col=None)
				hcF.to_csv(sheet.replace(sheet[sheet.index('.'):], '') + '.csv', sep=',', encoding='UTF-8')
			elif 'for reporting' in sheet:
				print('  [*]', sheet, end='  ----> ')
				try:
					forF = pd.read_excel(sheet, 'Conso', index_col=None)
					forF.to_csv(sheet.replace(sheet[sheet.index('.'):], '') + '.csv', sep=',', encoding='UTF-8')
				except XLRDError:
